...
(define-type Url net/url-structs:url)

(define-type Out-Format (U 'single-line 'multi-line))

(define-type Timeline-Order (U 'old->new 'new->old))

(struct Msg
        ([ts-epoch : Integer] [ts-orig : String]
                              [nick : (Option String)]
                              [uri : Url]
                              [text : String]
                              [mentions : (Listof Peer)]))

(struct Peer ([nick : (Option String)] [uri : Url])
  #:transparent)
      ...
      (define (work)
        ...
        (match (thread-receive)
          ...
          [(cons 'unit x) (begin
                            (define y (f x))
                            (when y
                              (thread-send parent (cons 'result y)))
                            (work))]))
      (work)))
...
(define msg-print
  (let* ([colors (vector 36 33)] [n (vector-length colors)])
    (λ (out-format color-i msg)
      (let ([color (vector-ref colors (modulo color-i n))]
        ...
        (match out-format
          ['single-line (let ([nick (if nick nick uri)])
                          (printf "~a  \033[1;37m<~a>\033[0m  \033[0;~am~a\033[0m~n"
                                  ...
                                  color
                                  text))]
          ['multi-line (let ([nick (if nick (string-append nick " ") "")])
...
(define rfc3339->epoch
  (let ([re
         (pregexp
          "^([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9]{2}):([0-9]{2})(:([0-9]{2}))?(\\.[0-9]+)?(Z|([+-])([0-9]{1,2}):?([0-9]{2}))?$")])
    (λ (ts)
  ...
  (let ([re (pregexp "^([^\\s\t]+)[\\s\t]+(.*)$")])
    (λ (nick uri str)
      (with-handlers*
       ([exn:fail?
         (λ (e)
           (log-error "Failed to parse msg: ~v, from: ~v, at: ~v, because: ~v"
                      str
                      nick
                      (url->string uri)
                      e)
       ...
       (match (regexp-match re str)
         [(list _wholething ts-orig text)
          (let ([ts-epoch (rfc3339->epoch ts-orig)])
            (if ts-epoch
                (let ([mentions (filter-map (λ (m)
                                              (match (regexp-match #px"@<([^>]+)>" m)
                                                [(list _wholething nick-uri) (str->peer nick-uri)]))
                                            (regexp-match* #px"@<[^\\s]+([\\s]+)?[^>]+>" text))])
                  (Msg ts-epoch ts-orig nick uri text mentions))
                (begin
                  (log-error "Msg rejected due to invalid timestamp: ~v, nick:~v, uri:~v"
                             str
                             nick
                             (url->string uri))
                  #f)))]
...
(module+ test
  ...
  (let* ([tzs (for*/list ([d '("-" "+")] [h '("5" "05")] [m '("00" ":00" "57" ":57")])
                (string-append d h m))]
         [tzs (list* "" "Z" tzs)])
    (for* ([n '("fake-nick")]
           [u '("fake-uri")]
      ...
      (let* ([ts (string-append "2020-11-18T22:22" (if (non-empty-string? s) s ":00") z)]
  ...
  (let* ([ts "2020-11-18T22:22:09-0500"]
    ...
    (check-equal? (Msg-ts-epoch actual) (Msg-ts-epoch expected) "str->msg ts-epoch")
    (check-equal? (Msg-ts-orig actual) (Msg-ts-orig expected) "str->msg ts-orig")
    (check-equal? (Msg-nick actual) (Msg-nick expected) "str->msg nick")
    (check-equal? (Msg-uri actual) (Msg-uri expected) "str->msg uri")
    (check-equal? (Msg-text actual) (Msg-text expected) "str->msg text")))

(: str->lines (-> String (Listof String)))
(define (str->lines str) (string-split str (regexp "[\r\n]+")))
...
(define (url->cache-file-path-v2 uri) (build-path cache-object-dir (uri-encode (url->string uri))))

(define url->cache-object-path url->cache-file-path-v2)

(define (url->cache-etag-path uri) (build-path cache-dir "etags" (uri-encode (url->string uri))))

(define (url->cache-lmod-path uri) (build-path cache-dir "lmods" (uri-encode (url->string uri))))
...
(: uri? (-> String Boolean))
(define (uri? str) (regexp-match? #rx"^[a-z]+://.*" (string-downcase str)))
...
(define (filter-comments lines) (filter-not (λ (line) (string-prefix? line "#")) lines))

(: str->peers (-> String (Listof Peer)))
(define (str->peers str) (filter-map str->peer (filter-comments (str->lines str))))

(: peers->file (-> (Listof Peers) Path-String Void))
(define (peers->file peers path)
  (display-lines-to-file
   (map (match-lambda [(Peer n u) (format "~a~a" (if n (format "~a " n) "") (url->string u))]) peers)
   path
   #:exists 'replace))
...
(define (b->n b) (string->number (bytes->string/utf-8 b)))
...
(define (rfc2822->epoch timestamp)
  (match (regexp-match re-rfc2822 timestamp)
    [(list _ _ dd mo yyyy HH MM SS)
     ...
     (find-seconds (b->n SS) (b->n MM) (b->n HH) (b->n dd) (mon->num mo) (b->n yyyy) #f)]
    [_ #f]))
...
(define user-agent
  (let* ([prog-name "tt"]
         ...
         [user (if (file-exists? user-peer-file)
...
(define (uri-download u)
  ...
  (define status (string->number (second (string-split (bytes->string/utf-8 status-line)))))
  ...
  (match status
    [200
     (let* ([etag (header-get headers #"ETag")]
            ...
            [lmod-prev
             (if (file-exists? cached-lmod-path) (rfc2822->epoch (file->bytes cached-lmod-path)) #f)])
       ...
       (unless (or (and etag
                   ...
                   (and lmod-curr
                        ...
                        (<= lmod-curr lmod-prev)
                        (begin
                          (log-info "Last-Modified <= current skipping the rest of ~v"
                                    (url->string u))
                          #t)))
         (begin
           (log-info "Downloading the rest of ~v. ETag: ~a, Last-Modified: ~v"
                     (url->string u)
                     etag
                     lmod)
           ...
           (call-with-output-file cached-object-path (curry copy-port body-input) #:exists 'replace)
           (when etag
...
(define (timeline-print out-format timeline)
  (void (foldl (match-lambda** [((and m (Msg _ _ nick _ _ _)) (cons prev-nick i))
   ...
   ([exn:fail?
    ...
    [integer?
     (λ (status)
       ...
       #f)])
   (define-values (_result _tm-cpu-ms tm-real-ms _tm-gc-ms) (time-apply uri-download (list uri)))
...
(define (uniq xs) (set->list (list->set xs)))

(: peers->timeline (-> (listof Peer) (listof Msg)))
(define (peers->timeline peers) (append* (filter-map peer->msgs peers)))
...
(module+ main
  (let ([log-level 'info])
    (command-line
     #:program "tt"
     #:once-each [("-d" "--debug") "Enable debug log level." (set! log-level 'debug)]
     #:help-labels ""
     ...
     (match command
       [(or "d" "download")
        ...
        (let ([num-workers 5])
          (command-line #:program "tt download"
                        #:once-each [("-j" "--jobs")
                                     njobs
       ...
       [(or "u" "upload")
        (command-line
         #:program "tt upload"
         #:args ()
         (if (system (path->string (build-path tt-home-dir "upload"))) (exit 0) (exit 1)))]
       [(or "r" "read")
        (let ([out-format 'multi-line] [order 'old->new] [ts-min #f] [ts-max #f])
          (command-line
           #:program "tt read"
           #:once-each [("-r" "--rev") "Reverse displayed timeline order." (set! order 'new->old)]
           [("-m" "--min")
            m
            "Earliest time to display (ignore anything before it)."
            (set! ts-min (rfc3339->epoch m))]
           [("-x" "--max")
            x
            "Latest time to display (ignore anything after it)."
            (set! ts-max (rfc3339->epoch x))]
           #:once-any [("-s" "--short") "Short output format" (set! out-format 'single-line)]
           [("-l" "--long") "Long output format" (set! out-format 'multi-line)]
           #:args file-paths
           (let* ([peers (paths->peers file-paths)]
                  [timeline (timeline-sort (peers->timeline peers) order)]
                  [timeline (filter (λ (m)
                                      (and (if ts-min (>= (Msg-ts-epoch m) ts-min) #t)
                                           (if ts-max (<= (Msg-ts-epoch m) ts-max) #t)))
                                    timeline)])
       ...
       [(or "c" "crawl")
        (command-line
         #:program "tt crawl"
         #:args file-paths
         (let* ([peers-sort (λ (peers)
                              (sort peers
                                    (match-lambda** [((Peer n1 _) (Peer n2 _))
                                                     (string<? (if n1 n1 "") (if n2 n2 ""))])))]
                [peers-all-file (build-path tt-home-dir "peers-all")]
                [peers-mentioned-file (build-path tt-home-dir "peers-mentioned")]
                [peers (paths->peers (match file-paths
                                       ; TODO Refactor such that path->string not needed
                                       ['() (list (path->string peers-all-file))]
                                       [_ file-paths]))]
                [timeline (peers->timeline peers)]
                [peers-mentioned-curr (uniq (append* (map Msg-mentions timeline)))]
                [peers-mentioned-prev (file->peers peers-mentioned-file)]
                [peers-mentioned (peers-sort (uniq (append peers-mentioned-prev
                                                           peers-mentioned-curr)))]
                [peers-all-prev (file->peers peers-all-file)]
                [peers-all (list->set (append peers peers-mentioned peers-all-prev))]
                [n-peers-discovered (set-count (set-subtract peers-all (list->set peers-all-prev)))]
                [peers-all (peers-sort (set->list peers-all))])
           (log-info "Discovered ~a new peers." n-peers-discovered)
           (peers->file peers-mentioned peers-mentioned-file)
           (peers->file peers-all peers-all-file)))]
       [command
